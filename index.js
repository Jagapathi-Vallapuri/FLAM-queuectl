...
export const HOME = process.env.QUEUECTL_HOME ? path.resolve(process.env.QUEUECTL_HOME) : path.join(os.homedir(), '.queuectl');
...
export function getStatus() {
    const d = db();
    const jobCounts = Object.fromEntries(d.prepare('SELECT state, COUNT(*) c FROM jobs GROUP BY state').all().map(r => [r.state, r.c]));
    const dlqCount = d.prepare('SELECT COUNT(*) c FROM dlq').get().c;
    const counts = { ...jobCounts, dead: dlqCount };
